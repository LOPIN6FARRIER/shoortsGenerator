...
export async function saveTopic(
  ...
  const db = getDatabase();
  await db.query(
    `INSERT INTO topics (id, title, description, execution_id, openai_model, openai_tokens_used, raw_response, generated_at)
...
export async function getRecentTopics(limit: number = 50): Promise<Topic[]> {
  ...
  const result = await db.query<Topic>(
    "SELECT id, title, description, generated_at as timestamp FROM topics ORDER BY generated_at DESC LIMIT $1",
    [limit],
  );
  return result.rows;
}
...
export async function getTopicById(topicId: string): Promise<Topic | null> {
  const db = getDatabase();
  const result = await db.query<Topic>(
    "SELECT id, title, description, generated_at as timestamp FROM topics WHERE id = $1",
    [topicId],
  );
  return result.rows[0] || null;
}
